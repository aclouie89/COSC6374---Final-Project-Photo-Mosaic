	...
	for (size_t y = 0; y < mosaic.height; y++)
	{
		for (size_t x = 0; x < mosaic.width; x++)
		{
			...
			for(int n = 0; n < components.size(); n++)
				if ((x >= tiles.at(n).start_x && y >= tiles.at(n).start_y) && (x <= tiles.at(n).end_x && y <= tiles.at(n).end_y))
					image.set_pixel(x, y, tiles.at(n).rgb);
			if (x == 0)
				printf("XY %d %d\n", x, y);
		}
	}
...
{
	mosaic_tile cur_tile = tiles.at(tile_index);
	bitmap_image final_img(FILE_OUT);
	bitmap_image tile_img(components.at(cur_tile.img_index).path);
	bitmap_image buffer;
...
{
	int n = 0;
	int img_index = tile_map.at(tile_index).at(n).index;

	// choose first best pick that isnt repeated as much
	while (n < components.size() && components.at(img_index).placed >= TILE_RPT_COUNT) {
		img_index = tile_map.at(tile_index).at(n).index;
		n++;
	}
...
int fit_check_repeated(unsigned int tile_index, unsigned int img_index)
{
	...
	for (int row = -TILE_MIN_DIST; row <= TILE_MIN_DIST; row++)
		for (int col = -TILE_MIN_DIST; col <= TILE_MIN_DIST; col++) {
			...
			if (loc < tiles.size() && loc >= 0)
				if (tiles.at(loc).img_index == img_index)
					seen = 1;
		}
...
int fit_best_pick_sparse(unsigned int tile_index)
{
	...
	while (n < components.size() && (components.at(img_index).placed >= TILE_RPT_COUNT  || seen)) {
		img_index = tile_map.at(tile_index).at(n).index;
...
void tile_place_best_fit(unsigned int tile_index)
{
	...
	tiles.at(tile_index).img_index = img_index;
	components.at(img_index).placed++;
}
...
{
	//tiles.at(tile_index)
	tile_map.at(tile_index).resize(components.size());

	string output = "Ranking Tile " + to_string(tile_index) + " Thread: " + to_string(omp_get_thread_num());
	dbgprint(2, output);

	// note we can have more images than tiles we place
	// for each image, check their weighting and add it into the list
	for(int n = 0; n < components.size(); n++) {
		int r = (int)components.at(n).rgb.red - (int)tiles.at(tile_index).rgb.red;
		int g = (int)components.at(n).rgb.green - (int)tiles.at(tile_index).rgb.green;
		int b = (int)components.at(n).rgb.blue - (int)tiles.at(tile_index).rgb.blue;

		// take root mean square average (simple average results in image being dark)
		tile_map.at(tile_index).at(n).index = n;
		// we want the absolute distance (0 being most preferred match)
		tile_map.at(tile_index).at(n).value = abs(r + g + b);
	}

	// biggest bottle neck for high tile images
	sort(tile_map.at(tile_index).begin(), tile_map.at(tile_index).end(), compare_tiles);
}
...
int get_component_file_weight()
{
		...
		for (int i = 0; i < n; i++) {
			...
			case DT_REG:
				...
				if (img < components.size()) {
					...
					g = sqrt(g / count);
					b = sqrt(b / count);
					components.at(img).rgb.red = r;
					components.at(img).rgb.green = g;
					components.at(img).rgb.blue = b;
	...
	string output = "Done finding weight for " + to_string(components.size()) + " component images";
	dbgprint(1, output);
...
int main()
{
	...
	num_imgs = get_component_file_list();
	if (TIMESTEPS) {
